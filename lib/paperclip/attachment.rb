...
module Paperclip
  ...
  class Attachment
    ...
    attr_reader :name, :instance, :default_style, :convert_options, :queued_for_write, :whiny,
                :options, :original_style, :interpolator, :source_file_options
    ...
    def save
      ...
      if process.any? && !process.include?(original_style)
        @queued_for_write.except!(original_style)
      end
      flush_writes
      @dirty = false
      true
    ...
    def reset_file_if_original_reprocessed
      ...
      assign_fingerprint { @queued_for_write[original_style].fingerprint }
      reset_updater
    end
    ...
    def post_process_style(name, style) #:nodoc:
      begin
        ...
        intermediate_files = []
        original = @queued_for_write[original_style]
